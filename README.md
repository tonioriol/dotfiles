...
- **macOS**: These dotfiles are designed for macOS (tested on Apple Silicon)
- **Git**: Pre-installed on macOS (will prompt to install Xcode Command Line Tools on first use)
- **Homebrew**: Will be installed automatically by `brew.sh` if not present
